                          ...
                          <div>
                            ...
                            <RadioGroup v-model="selectedColor" class="mt-4">
                              ...
                              <div class="flex items-center space-x-3">
                                <RadioGroupOption v-for="color in product.colors" :key="color.name" :value="color" :class="[color.selectedClass, 'relative -m-0.5 flex cursor-pointer items-center justify-center rounded-full p-0.5 focus:outline-none']">
                                  <RadioGroupLabel as="span" :class="color.name" class="sr-only">{{ color.name }}</RadioGroupLabel>
                          ...
                          <div class="mt-10">
                            ...
                            <RadioGroup v-model="selectedSize" class="mt-4">
                              ...
                              <div class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                                <RadioGroupOption v-for="size in product.sizes" :key="size.name" :value="size" :disabled="!size.inStock" :class="[size.inStock ? 'cursor-pointer bg-white text-gray-900 shadow-sm' : 'cursor-not-allowed bg-gray-50 text-gray-200', 'group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1']">
                                  <RadioGroupLabel as="span">{{ size.name }}</RadioGroupLabel>
                                  <span aria-hidden="true" v-if="!size.inStock" class="pointer-events-none absolute -inset-px rounded-md border-2 border-gray-200">
